...
5. **Candidate selection with serendipity** — ~80% genre-matched candidates, ~20% popular tracks from outside those genres to avoid filter bubbles.
6. **Euclidean-distance ranking** — candidates ranked by L2 distance to the target vector.
7. **Categorical re-ranking** — boosts/penalties from learned genre / country / artist-type / decade preferences.
